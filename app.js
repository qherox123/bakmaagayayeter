...
module.exports = async (client) => {
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      client.channels.cache
        ...
        .send(
          `<a:maintence:833375738547535913> vCodes has been switched to __maintance__ due to **${req.body.reason}**`
        )
        .then((a) => {
          new maintenceSchema({
            server: settingsc.serverID,
  ...
  app.post(
    ...
    async (req, res) => {
      ...
      const bakimsonaerdikardesvcodes = new dc.MessageEmbed()
        ...
        .setColor("GREEN")
        .setDescription(
          `<a:online:833375738785824788> vCodes are **active** again!\n[Click to redirect website](https://vcodes.xyz)`
        )
      ...
      await client.channels.cache
        ...
        .then((a) => {
          a.edit(
            `~~ <a:maintence:833375738547535913> vCodes has been switched to __maintance__ due to **${bakimdata.reason}** ~~`,
  ...
  app.get("/api/bots/check/:userID", async (req, res) => {
    ...
    if (!req.params.userID)
      return res.json({ error: "You must enter a user id." });
    const botdata = await botsdata.findOne({ token: token });
    if (!botdata)
      return res.json({ error: "You entered an invalid bot token." });
  ...
  app.post("/api/bots/stats", async (req, res) => {
    ...
    if (botdata) {
      return await botsdata.update(
        { botID: botdata.botID },
        { $set: { serverCount: req.header("serverCount") } }
      );
    }
  });
  //------------------- API -------------------//    //------------------- API -------------------//
